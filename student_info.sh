...
echo "$($PSQL "select course from courses full join majors_courses using(course_id) full join majors using(major_id) full join students using(major_id) where students.student_id is null or (students.first_name='Obie' and students.last_name='Hilpert') group by course order by course desc")"
